...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractFeatureFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            JSONArray earthquakesArray = jsonRootObject.optJSONArray("features");

            //iterate throw the array to get each earthquake instance.
            for (int i=0;i<earthquakesArray.length();i++) {
                JSONObject earthquake = earthquakesArray.getJSONObject(i);
                earthquake = earthquake.getJSONObject("properties");

                double mag = earthquake.optDouble("mag");
                String place = earthquake.optString("place");
                long date = earthquake.optLong("time");
                String url = earthquake.optString("url");
